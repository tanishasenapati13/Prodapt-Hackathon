...
def load_raw_jd():
    if os.path.exists(INPUT_JD_FILE):
        with open(INPUT_JD_FILE, "r", encoding="utf-8") as f:
            content = f.read().strip()
            if content:
                return content
    return """We are looking for a Data Scientist to analyze large amounts of raw information to find patterns that will help improve our company. We will rely on you to build data products to extract valuable business insights.
...
def extract_jd_information(raw_jd):
    prompt = f"""You are an expert recruitment information extraction system.
...
- Normalize obvious aliases where appropriate.
  Example:
    ...
    React.js -> React
    Node.js -> Node.js
- Do not treat generic words such as "teamwork", "communication", or "problem solving" as technical skills.
...
"""
    ...
    response = client.chat.completions.create(
        model=GROQ_MODEL,
        messages=[
            {"role": "system", "content": "You extract structured recruitment information from job descriptions."},
...
def normalize_skill(skill):
    ...
    return replacements.get(skill.lower().strip(), skill)
...
def clean_jd_data(jd_data, raw_jd):
    list_fields = [
        "required_skills", "preferred_skills", "required_tools", "preferred_tools",
        "required_qualifications", "preferred_qualifications", "responsibilities",
        "soft_skills", "domain_keywords", "education", "certifications"
    ]
    ...
    jd_data["required_skills"] = normalize_skill_list(jd_data["required_skills"])
    jd_data["preferred_skills"] = normalize_skill_list(jd_data["preferred_skills"])
    jd_data["required_tools"] = normalize_skill_list(jd_data["required_tools"])
    jd_data["preferred_tools"] = normalize_skill_list(jd_data["preferred_tools"])
...
def create_final_jd(jd_data):
    return {
        "job_description_id": "jd_" + datetime.now(timezone.utc).strftime("%Y%m%d%H%M%S"),
        ...
        "required_tools": jd_data.get("required_tools", []),
        "preferred_tools": jd_data.get("preferred_tools", []),
        ...
        "domain_keywords": jd_data.get("domain_keywords", []),
...
def save_jd(jd):
    os.makedirs(os.path.dirname(OUTPUT_JD_FILE), exist_ok=True)
    with open(OUTPUT_JD_FILE, "w", encoding="utf-8") as f:
...
def main():
    ...
    extracted_data = extract_jd_information(raw_jd)

    print("Cleaning extracted data...")
    cleaned_data = clean_jd_data(extracted_data, raw_jd)
    final_jd = create_final_jd(cleaned_data)
